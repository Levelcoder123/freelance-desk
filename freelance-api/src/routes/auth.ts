...
// ── POST /auth/login
authRouter.post('/login', authLimiter as any, validate(authValidation.loginSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// ── GET /auth/me
authRouter.get('/me', authenticate as any, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
authRouter.post('/forgot-password', authLimiter as any, validate(authValidation.forgotPasswordSchema), async (req: Request, res: Response, next: NextFunction) => {
...
authRouter.post('/reset-password', authLimiter as any, validate(authValidation.resetPasswordSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
authRouter.post('/refresh', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const payload = jwt.verify(refresh_token, process.env.JWT_REFRESH_SECRET as string) as jwt.JwtPayload;
...
// ── POST /auth/logout
authRouter.post('/logout', authenticate as any, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });
