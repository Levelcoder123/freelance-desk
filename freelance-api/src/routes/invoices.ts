...
invoicesRouter.use(authenticate as any);

// ── GET /invoices
invoicesRouter.get('/', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });
    const result = await invoiceService.getInvoices(req.userId, req.query as any);
    res.json(result);
  } catch (err) { next(err); }
});

// ── POST /invoices
invoicesRouter.post('/', validate(invoiceValidation.invoiceSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });
...
// ── GET /invoices/:id
invoicesRouter.get('/:id', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });
...
invoicesRouter.patch('/:id', validate(invoiceValidation.updateSchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const invoice = await invoiceService.updateInvoice(req.userId, req.params.id, updates as Partial<Invoice>);
    if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
    res.json(invoice);
  } catch (err) { next(err); }
});
...
invoicesRouter.post('/:id/send', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.userId) return res.status(401).json({ error: 'Unauthorized' });
